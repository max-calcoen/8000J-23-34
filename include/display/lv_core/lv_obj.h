...
typedef bool (*lv_design_func_t)(struct _lv_obj_t *obj, const lv_area_t *mask_p,
 ...
 * @param copy pointer to a base object, if not NULL then the new object will be
 * copied from it
 ...
 * @param y_mod y coordinate shift after alignment
 */
void lv_obj_align_origo(lv_obj_t *obj, const lv_obj_t *base, lv_align_t align,
                        lv_coord_t x_mod, lv_coord_t y_mod);
 ...
 * @param type type of animation from 'lv_anim_builtin_t'. 'OR' it with ANIM_IN
 * or ANIM_OUT
...
/**
 * Return with the system layer. (Same on every screen and it is above the all
 ...
 * @param obj pointer to an object
 * @return true: the object is focused, false: the object is not focused or not
 * in a group
 */
