...
public:
   ...
   */
  virtual Output
  runUntilSettled(const Input itarget,
                  IterativeController<Input, Output> &icontroller,
                  ControllerOutput<Output> &ioutput) {
    LOG_INFO("ControllerRunner: runUntilSettled(IterativeController): Set "
             "target to " +
             std::to_string(itarget));
    icontroller.setTarget(itarget);
    ...
    LOG_INFO("ControllerRunner: runUntilSettled(IterativeController): Done "
             "waiting to settle");
    return icontroller.getError();
  }

  /**
   * Runs the controller until it has reached its target, but not necessarily
   * settled.
   *
   * @param itarget the new target
   * @param icontroller the controller to run
  ...
  virtual Output runUntilAtTarget(const Input itarget,
    ...
    while (error != 0 &&
           std::copysign(1.0, error) == std::copysign(1.0, lastError)) {
      lastError = error;
      rate->delayUntil(10_ms);
      error = icontroller.getError();
    }

    LOG_INFO("ControllerRunner: runUntilAtTarget(AsyncController): Done "
             "waiting to settle");
    return icontroller.getError();
  }
   ...
   */
  virtual Output
  runUntilAtTarget(const Input itarget,
                   IterativeController<Input, Output> &icontroller,
                   ControllerOutput<Output> &ioutput) {
    LOG_INFO("ControllerRunner: runUntilAtTarget(IterativeController): Set "
             "target to " +
             std::to_string(itarget));
    icontroller.setTarget(itarget);
    ...
    LOG_INFO("ControllerRunner: runUntilAtTarget(IterativeController): Done "
             "waiting to settle");
    return icontroller.getError();
  }
