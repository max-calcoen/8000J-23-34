...
public:
   ...
   *
   * @param radians true for theta in radians, false for degrees. False by
   * default
   * @return lemlib::Pose
   */
   ...
   *
   * @note Units are in inches if curret motion is moveTo or follow, degrees if
   * using turnTo
   *
   ...
   * @param async whether the function should be run asynchronously. false by
   * default
   * @param reversed whether the robot should turn to face the point with the
   * back of the robot. false by default
   ...
   * @param async whether the function should be run asynchronously. false by
   * default
   * @param forwards whether the robot should move forwards or backwards. true
   * for forwards (default), false for backwards
   * @param lead the lead parameter. Determines how curved the robot will move.
   * 0.6 by default (0 < lead < 1)
   * @param chasePower higher values make the robot move faster but causes more
   * overshoot on turns. 0 makes it default to global value
