...
public:
  ...
  Gps(const std::uint8_t port, double xInitial, double yInitial,
      ...
      : _port(port) {
    pros::c::gps_initialize_full(port, xInitial, yInitial, headingInitial,
                                 xOffset, yOffset);
  };
   ...
   * center of turning (meters) \param  yOffset Pointer to cartesian 4-Quadrant
   * Y offset from center of turning (meters) \return 1 if the operation was
   ...
   */
  virtual std::int32_t set_position(double xInitial, double yInitial,
