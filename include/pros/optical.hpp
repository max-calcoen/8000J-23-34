...
public:
   ...
   * with minimum time being 3 ms and maximum time being 712 ms. Default is 100
   * ms, with the optical sensor communciating with the V5 brain every 20 ms.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENXIO - The given value is not within the range of V5 ports (1-21).
   * ENODEV - The port cannot be configured as an Optical Sensor
   *
   * \return 1 if the operation is successful or PROS_ERR_F if the operation
   * failed, setting errno.
   */
