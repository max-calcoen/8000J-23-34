...
public:
   ...
   */
  [[deprecated("Changing these values is not supported by VEX and may lead to "
               "permanent motor damage.")]] static motor_pid_s_t
  ...
  convert_pid_full(double kf, double kp, double ki, double kd, double filter,
   ...
   */
  [[deprecated("Changing these values is not supported by VEX and may lead to "
               "permanent motor damage.")]] virtual std::int32_t
...
public:
  ...
  explicit Motor_Group(const std::vector<std::int8_t>
                           motor_ports); // Pass by value to preserve ABI
   ...
   * pros::Motor::set_zero_position().
   *
   * \note This function simply sets the target for the motors, it does not
   ...
   * EACCESS - The Motor group mutex can't be taken or given
   *
   * \return The voltage of the motor in millivolts or PROS_ERR_F if the
   * operation failed, setting errno.
   *
   * \b Example
   * \code
   ...
   * EACCESS - The Motor group mutex can't be taken or given
   *
   * \return The voltage limit of the motor in millivolts or PROS_ERR_F if the
   * operation failed, setting errno.
   *
   * \b Example
   * \code
   ...
   * EACCESS - The Motor group mutex can't be taken or given
   *
   * \return A vector of the raw encoder positions of the motors in the motor
   *group based on the timestamps passed in. If a timestamp is not found for a
   *motor, the value at that index will be PROS_ERR.
   *
   * \b Example
   * \code
   * void opcontrol() {
  ...
  get_raw_positions(std::vector<std::uint32_t *> &timestamps);
  /****************************************************************************/
  /**                      Motor configuration functions                     **/
  /**                                                                        **/
  ...
  /**
   * Sets the "absolute" zero position of the motor group to its current
   * position.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   * EACCESS - The Motor group mutex can't be taken or given
   *
   * \return The vector filled with motors' temperature in degrees Celsius or
