...
/** type definition for the pair of smart port and adi port for the basic adi
 * devices */
using ext_adi_port_pair_t = std::pair<std::uint8_t, std::uint8_t>;

/** type definition for the triplet of smart port and two adi ports for the two
 * wire adi devices*/
using ext_adi_port_tuple_t =
...
public:
   ...
   */
  explicit ADIPort(std::uint8_t adi_port,
   ...
   *
   * \param port_pair
   *        The pair of the smart port number (from 1-22) and the ADI port
   *number (from 1-8, 'a'-'h', 'A'-'H') to configure \param type The
   *configuration type for the port
   */
  ADIPort(ext_adi_port_pair_t port_pair,
          adi_port_config_e_t type = E_ADI_TYPE_UNDEFINED);
...
public:
   ...
   *        The value to set the ADI port to
   *
   * \return if the operation was successful or PROS_ERR if the operation
   * failed, setting errno.
   */
  using ADIPort::set_value;
};
...
public:
   ...
   *side up \param adi_port_bottom The "bottom" wire from the encoder sensor
   * \param reverse
   *        If "true", the sensor will count in the opposite direction
   */
  ADIEncoder(std::uint8_t adi_port_top, std::uint8_t adi_port_bottom,
             bool reversed = false);
   ...
   * 		  sensor with the removable cover side up, and the "bottom" wire
   *from the encoder sensor \param reverse If "true", the sensor will count in
   *the opposite direction
   */
  ADIEncoder(ext_adi_port_tuple_t port_tuple, bool reversed = false);

  // Delete copy constructor to prevent a compilation error from the constructor
  // above.
...
public:
   ...
   *port 1, 3, 5, or 7 ('A', 'C', 'E', 'G'). \param port_echo The port connected
   *to the yellow INPUT cable. This should be in the next highest port following
   *port_ping.
   */
...
public:
   ...
   * port has not previously been configured as a gyro, then this function
   *starts a 1300ms calibration period.
   *
   * It is highly recommended that an ADIGyro object be created in initialize()
   * when the robot is stationary to ensure proper calibration. If an ADIGyro
   * object is declared at the global scope, a hardcoded 1300ms delay at the
...
public:
   ...
   *        An adi_potentiometer_type_e_t enum value specifying the
   *potentiometer version type
   */
  ADIPotentiometer(
      std::uint8_t adi_port,
      adi_potentiometer_type_e_t potentiometer_type = E_ADI_POT_EDR);
   ...
   * The original potentiometer rotates 250 degrees thus returning an angle
   between 0-250 degrees.
   * Potentiometer V2 rotates 330 degrees thus returning an angle between 0-330
   degrees.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   ...
   * calibration value.
   *
   * This method assumes that the potentiometer value is not actively changing
   * at this time and computes an average from approximately 500 samples, 1 ms
   * apart, for a 0.5 s period of calibration. The average value thus calculated
   * is returned and stored for later calls to the
   ...
   * when called.
   *
   * Do not use this function when the potentiometer value might be unstable
   ...
   * ENODEV - The port is not configured as a potentiometer
   *
   * \return The difference of the potentiometer value from its calibrated
   * default from -4095 to 4095
   */
