...
public:
  ...
  static task_t create(F &&function, std::uint32_t prio = TASK_PRIORITY_DEFAULT,
    ...
    return pros::c::task_create(
        ...
        new std::function<void()>(std::forward<F>(function)), prio, stack_depth,
        name);
  }
   ...
   *
   */
  template <class F> static task_t create(F &&function, const char *name) {
    return Task::create(std::forward<F>(function), TASK_PRIORITY_DEFAULT,
   ...
   */
  explicit operator task_t() { return task; }
...
public:
   ...
   * 		 std::scoped_lock instead of interacting with the Mutex
   *directly.
   *
  ...
  try_lock_until(const std::chrono::time_point<Clock, Duration> &abs_time) {
    return take(
        std::max(static_cast<uint32_t>(0), (abs_time - Clock::now()).count()));
  }
};
...
public:
   ...
   */
  constexpr Var &operator*() const { return var; }
...
public:
   ...
   */
  template <typename Rep, typename Period>
  std::optional<MutexVarLock<Var>>
  try_lock(const std::chrono::duration<Rep, Period> &rel_time) {
 ...
 *
 * \param milliseconds
 * 		  The number of milliseconds to wait (1000 milliseconds per
 * second)
 */
using pros::c::delay;
} // namespace pros
