...
public:
   ...
   * \param link_id
   *      Unique link ID in the form of a string, needs to be different from
   * other links in the area. \param type Indicates whether the radio link on
   * the brain is a transmitter or reciever, with the transmitter having double
   * the transmitting bandwidth as the recieving end (1040 bytes/s vs 520
   * bytes/s). \param ov Indicates if the radio on the given port needs vexlink
   * to override the controller radio
   *
   * \return PROS_ERR if initialization fails, 1 if the initialization succeeds.
   */
  Link(const std::uint8_t port, const std::string link_id, link_type_e_t type,
       bool ov = false);
   ...
   * radio.
   *
   * \return PROS_ERR if port is not a link/radio, else the bytes available to
   * be read by the user.
   */
   ...
   * transmission, and there is no room in the FIFO buffer (queue) to transmit
   * the data. EINVAL - The data given is NULL
   *
   * \param data
   ...
   * ENXIO - The sensor is still calibrating, or no link is connected via the
   radio.
   * EINVAL - The destination given is NULL, or the size given is larger than
   the FIFO buffer
